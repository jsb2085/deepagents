...
def save_auth_mode(auth_mode: str, config_path: Path | None = None) -> bool:
    ...
    the canonical mode verbatim. Bypasses `_save_model_field` deliberately:
    the mode is not a model spec, so the `models.allowed` policy must not
    gate it.
    ...
    """
    return _save_toml_field("models", "auth_mode", auth_mode, config_path)
